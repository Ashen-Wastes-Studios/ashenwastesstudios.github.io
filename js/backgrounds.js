...
(function() {
  ...
  // Draw neural connections between nearby galaxies using their actual rendered positions
  function drawNeuralConnections(visibleGalaxies) {
    ...
    for (let i = 0; i < visibleGalaxies.length; i++) {
      for (let j = i + 1; j < visibleGalaxies.length; j++) {
        ...
        // Calculate actual rendered positions (same as drawGalaxy)
  ...
  // AI/Ashen pages — floating data streams
  const dataStreams = Array.from({ length: 15 }, () => ({
    x: Math.random() * w, y: Math.random() * h, length: Math.random() * 80 + 40,
    speed: Math.random() * 1.5 + 0.5, chars: Array.from({ length: 8 }, () => String.fromCharCode(0x30A0 + Math.random() * 96)),
  ...
  const matrixDrops = Array.from({ length: 40 }, () => ({
    x: Math.random() * w, y: Math.random() * -h, speed: Math.random() * 2.5 + 1.5,
  ...
  // Gaming/Engine pages — 3D wireframe shapes
  let rotX = 0, rotY = 0;
  const shapes3D = Array.from({ length: 8 }, () => ({
    x: (Math.random() - 0.5) * w * 1.5, y: (Math.random() - 0.5) * h * 1.5, z: (Math.random() - 0.5) * 600,
  ...
  function drawWaves() {
    ...
    for (let layer = 0; layer < 5; layer++) {
      ...
      const offset = layer * 40 + scrollY * 0.05;
